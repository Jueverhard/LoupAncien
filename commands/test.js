...
module.exports = (client, message) => {
  ...
  if(message.channel.id != client.TestCommandesChanelID && message.channel.id != client.TestLogsChanelID) return;
  ...
  const date3 = moment().add(6, 'days').startOf('isoweek').subtract(1, 'days').format('DD/MM');
  var gvgMembers = [];
  var noGvgMembers = [];
  ...
  client.channels.fetch(client.GvGChanelID).then(chan => {
    chan.messages.fetchPinned().then(msgCollections => {
  ...
  setTimeout(function(){
    ...
    client.channels.fetch(client.TestLogsChanelID).then(chan => {
      chan.send(embed);
